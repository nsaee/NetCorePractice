        ...
        private static IDictionary<string, string> CreateAndSaveDefaultValues(ConfigurationDbContext dbContext)
        {
            var configValues =
                new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
                {
                    ...
                    { $"{nameof(CustomeBaseConfigData)}:{nameof(CustomeBaseConfigData.IsUnderTest)}", "true" },
                    {"CacheConfiguration:Type","DistributedSqlServerCache" },
                    {"CacheConfiguration:DistributedSqlServerCache:ConnectionString","server=.; database=CacheDB; Integrated Security=true" },
                    {"CacheConfiguration:DistributedSqlServerCache:TableName","CacheData" },
                    {"CacheConfiguration:DistributedSqlServerCache:SchemaName","dbo" },
                };
